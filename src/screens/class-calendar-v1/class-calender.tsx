...
const ClassCalendar = ({ role }: { role: string }) => {
  ...
  const classSchedulesQueryParams = useMemo(() => {
    return {
      day_of_week: "MON",
      tutor_ids: selectedTeacher?.id,
      student_ids: selectedStudent?.id,
      startDate: dateFilter[0] || "",
      endDate: dateFilter[1] || "",
      enrollment_id: selectedSubject?.id,
    };
  }, [role, user?.id, selectedTeacher, selectedStudent, selectedSubject]);
  ...
  const {
    ...
    queryKey: [
      "class-schedules-groupedByDay",
      selectedTeacher,
      selectedStudent,
      selectedSubject,
    ],
    queryFn: () => getAllClassSchedulesV1(classSchedulesQueryParams, { token }),
    enabled: !!token && !!user?.id,
    refetchOnWindowFocus: false,
    staleTime: 120000,
  });

  const { data, rescheduleData } = useMemo(
    () => classSchedulesData ?? { data: undefined, rescheduleData: undefined },
    [classSchedulesData],
  );

  // Refactor class schedule data (normal slots)
  ...
  // Refactor reschedule requests data (extra slots)
  const refactorResheduleRequests = useMemo(() => {
    if (!rescheduleData || !Array.isArray(rescheduleData)) return [];

    return rescheduleData.map((item: any) => ({
      type: "extra slot",
      id: item.id,
      start: moment.utc(item.DateTime).local().toDate(),
        ...
        .utc(item.DateTime)
        .local()
        .add(item.duration || 60, "minutes")
        .toDate(),
      status: item.class_status,
      title: ` ${
        item.enrollment_reschedual?.studentsGroups?.[0]?.user?.name
          ?.trim()
          .split(" ")[0] || "No Show"
      } x ${
        item.enrollment_reschedual?.tutor?.name?.trim().split(" ")[0] ||
        "No Show"
      }`,
      slotData: item,
    }));
  ...
  // Combine both into a single array for calendar
